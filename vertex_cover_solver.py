...
def kernelization(k, count):
    """
    INPUT: k is int, count is int
...
def vc_branch(k):
    ...
    S_kern, undelete, unmerge, k = kernelization(k, vc_branch.counter)
    if k < 0:
...
def vc():
    ...
    if is_edgeless(): S = []
        ...
        S_kern, _, _, _ = kernelization(nb_vertices - 1, vc_branch.counter)
        if is_edgeless(): S = S_kern
        else:
            x = bound()
            bound.counter += 1
