...
run_ce_solver()
{
	...
	FILES=$(ls random_super_small/*.input)

	if [ $6 -eq 1 ]; then
		FILES=$(ls -rS dimacs_super_small/*.dimacs)
	fi
	if [ $6 -eq 2 ]; then
		FILES=$(ls -rS snap_super_small/*.dimacs)
	fi
...
# 			time=$(cat time.txt | awk '/system/{print $3}' | sed -e 's/elapsed//');
			...
			echo -e $f"\t"$time"s\t"$k"\t"$recursiveSteps"\t"$firstLowerBoundDifference"\t"$highDegreeRules"\t"$degreeZeroRules"\t"$extremeReductionRules"\t"$degreeOneRules"\t"$lowerBounds"\t"$finished"\t"$verify
			echo "" >> $LOG
...
echo "run random instances $PROGRAMM_NAME (Tab-separated columns: File, Time in seconds, solution size, recursive steps, first lower bound difference, high degree rules, degree zero rules, extreme reduction rules, degree one rules, lower bounds, finished, solution size verified)"
run_ce_solver "$PROGRAMM_NAME" $LOG $maxSec $maxSecPerInstance $maxNotSolved 0

echo "run dimacs instances $PROGRAMM_NAME (Tab-separated columns: File, Time in seconds, solution size, recursive steps, first lower bound difference, high degree rules, degree zero rules, extreme reduction rules, degree one rules, lower bounds, finished, solution size verified)"
run_ce_solver "$PROGRAMM_NAME" $LOG $maxSec $maxSecPerInstance $maxNotSolved 1

echo "run snap instances $PROGRAMM_NAME (Tab-separated columns: File, Time in seconds, solution size, recursive steps, first lower bound difference, high degree rules, degree zero rules, extreme reduction rules, degree one rules, lower bounds, finished, solution size verified)"
